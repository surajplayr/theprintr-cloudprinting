...
export default function Home() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFileSize(mb >= 1 ? `${mb.toFixed(1)} MB` : `${Math.round(totalBytes / 1024)} KB`);
      ...
      <main className="flex-1 p-5 overflow-y-auto pb-32 custom-scroll">
        ...
        {screen === "config" && (
          <section className="space-y-4 animate-fade-in-up">
            ...
            <div className="bg-white rounded-xl p-3 shadow-sm border border-noir/10 flex items-center justify-between hover:shadow-md transition-all duration-200">
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="w-10 h-12 bg-cherry/10 border border-cherry/20 rounded-lg flex flex-col items-center justify-center text-cherry font-black text-[11px] shrink-0">
                  <FileText className="w-4 h-4 mb-0.5" />
                  <span>{fileType === "image" ? "IMG" : fileType === "mixed" ? "MIX" : "PDF"}</span>
                </div>
                <div className="truncate">
                  <p className="text-xs font-extrabold text-noir truncate max-w-[180px]">{fileName}</p>
                  <p className="text-[10px] text-noir/50 font-semibold mt-0.5">
                    {fileSize} &bull; {pages} {pages === 1 ? "page" : "pages"}
                  </p>
                </div>
              </div>
              <button
                ...
                className="w-8 h-8 rounded-full bg-noir/5 hover:bg-red-50 text-noir/50 hover:text-red-500 flex items-center justify-center transition-all duration-200 active:scale-[0.90] shrink-0"
              >
            ...
            {/* Setting: Color Mode */}
            <div className={`space-y-1.5 ${isUploading ? 'opacity-50 pointer-events-none' : ''} transition-opacity`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-2 px-3 rounded-xl border border-noir/15 font-bold text-xs flex flex-col items-center justify-center gap-0.5 transition-all active:scale-[0.98] ${
                    colorMode === "bw" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-xl border border-noir/15 font-bold text-xs flex flex-col items-center justify-center gap-0.5 transition-all active:scale-[0.98] ${
                    colorMode === "color" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
            ...
            <div className={`space-y-1.5 ${isUploading ? 'opacity-50 pointer-events-none' : ''} transition-opacity`}>
              ...
              <div className="w-full">
                <button
                  type="button"
                  className="w-full pill-active py-2.5 px-3 rounded-xl border border-noir/15 font-bold text-xs flex items-center justify-center gap-2 cursor-default shadow-sm"
                >
            ...
            <div className={`space-y-1.5 ${isUploading ? 'opacity-50 pointer-events-none' : ''} transition-opacity`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className={`py-2 px-2 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    orientation === "auto" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Auto
                </button>
                <button
                  ...
                  className={`py-2 px-2 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    orientation === "portrait" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Portrait
                </button>
                <button
                  ...
                  className={`py-2 px-2 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    orientation === "landscape" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Landscape
                </button>
            ...
            <div className={`space-y-1.5 ${isUploading ? 'opacity-50 pointer-events-none' : ''} transition-opacity`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className={`py-2 px-1 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    scaling === "fit" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Fit to page
                </button>
                <button
                  ...
                  className={`py-2 px-1 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    scaling === "actual" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Actual size
                </button>
                <button
                  ...
                  className={`py-2 px-1 rounded-xl border border-noir/15 font-bold text-xs text-center transition-all active:scale-[0.98] ${
                    scaling === "fill" ? "pill-active" : "bg-white text-noir hover:bg-cotton shadow-sm"
                  }`}
                >
                  Fill page
                </button>
            ...
            <div className={`bg-white rounded-2xl p-3 border border-noir/10 flex items-center justify-between shadow-sm transition-opacity ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
              <div>
        ...
        <footer className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-noir/10 px-4 py-3 z-20 flex items-center justify-between gap-3">
          ...
          <div className="flex flex-col gap-2 w-full max-w-[200px]">
            <button
              ...
              disabled={isSubmittingOnline || isSubmittingCash || isUploading}
              className="bg-cherry hover:bg-maroon active:scale-[0.98] transition-all text-cotton font-extrabold text-[11px] py-2 px-3 rounded-lg shadow-md flex items-center justify-center gap-1.5 w-full disabled:opacity-50 disabled:pointer-events-none"
            >
              {isSubmittingOnline ? (
                <>
            ...
            <button
              ...
              disabled={isSubmittingOnline || isSubmittingCash || isUploading}
              className="bg-transparent border border-cherry text-cherry hover:bg-cherry/5 active:scale-[0.98] transition-all font-extrabold text-[11px] py-1.5 px-3 rounded-lg flex items-center justify-center gap-1.5 w-full disabled:opacity-50 disabled:pointer-events-none"
            >
              {isSubmittingCash ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
